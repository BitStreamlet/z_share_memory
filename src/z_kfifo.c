...
// Writes data into the kfifo buffer.
uint32_t z_kfifo_shm_in(struct z_kfifo_struct *p_fifo, const void *p_from, uint32_t len, void *p_buf_addr) {
    if (!p_fifo || !p_from || !p_buf_addr) return 0;
    uint32_t l, off;
    uint8_t *p_buffer = (uint8_t *)p_buf_addr;
    ...
    // Determine how much data can be written in the first segment.
    l = Z_TOOL_MIN(len, p_fifo->size - off);
    memcpy(p_buffer + off, p_from, l); // Write to the buffer.

    // Write remaining data if the buffer wraps around.
    memcpy(p_buffer, (char *)p_from + l, len - l);
    p_fifo->in += len; // Update the write pointer.
    return len;        // Return the number of bytes written.
}

// Reads data from the kfifo buffer.
uint32_t z_kfifo_shm_out(struct z_kfifo_struct *p_fifo, void *p_to, uint32_t len, void *p_buf_addr) {
    if (!p_fifo || !p_to || !p_buf_addr) return 0;
    uint32_t off, l;
    uint8_t *p_buffer = (uint8_t *)p_buf_addr;
    ...
    // Determine how much data can be read in the first segment.
    l = Z_TOOL_MIN(len, p_fifo->size - off);
    memcpy(p_to, p_buffer + off, l); // Read from the buffer.

    // Read remaining data if the buffer wraps around.
    memcpy((char *)p_to + l, p_buffer, len - l);
    p_fifo->out += len; // Update the read pointer.
    return len;         // Return the number of bytes read.
}
